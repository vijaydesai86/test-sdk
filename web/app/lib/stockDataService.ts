...
const tryLoadGetCrumbClear = async (): Promise<void> => {
  ...
  try {
    ...
    // createRequire rooted at cwd so 'yahoo-finance2' resolves from node_modules
    // even when this file is bundled (webpack marks yf2 as external).
    const _req = createRequire(pathToFileURL(process.cwd() + '/package.json').href);
    const yf2CjsPath = _req.resolve('yahoo-finance2');
    // Derive the ESM getCrumb.js path from the CJS entry path.
...
const loadYahooModule = async (): Promise<YahooFinanceModule> => {
  try {
    ...
    // Load getCrumbClear now while the ESM module graph is in Node.js's cache,
    // so clearYahooCrumb() can properly reset getCrumb.js's internal singletons.
...
const clearYahooCrumb = async (): Promise<void> => {
  try {
    ...
    // Step 1: reset the module-level 'crumb' and 'promise' singletons so that
    // the next getCrumb() call starts a genuine new fetch instead of returning
    // the cached rejected Promise.
    if (getCrumbClearFn && instance._opts?.cookieJar) {
      await getCrumbClearFn(instance._opts.cookieJar);
    }
    // Step 2: install a fresh cookie jar so the new crumb fetch carries no
    // previously-set (expired or blocked) session cookies.
    ...
    while (attempt <= this.maxRetries) {
      try {
        ...
        if (isRateLimit) {
          ...
          // Gives Yahoo Finance's per-IP rate limiter time to reset, and
          // gives the fresh-session approach (new cookies + crumb) the best
