...
Deep-sector research resolves a broad live provider-confirmed candidate pool through generic theme role buckets. The LLM may propose company/ticker ideas with role evidence, but every symbol must still be provider-verified or searched by company name before data fetching. The selector persists role evidence and theme qualification metadata (`direct`, `enabler`, `beneficiary`, `unrelated`), applies evidence/fit gates before selection, locks the saveable company list, fetches critical market data (price, overview, basic financials, price history) before optional ecosystem/dependency LLM enrichment, and never lets later model passes silently change the locked universe. Universe scoring must remain generic: no production ticker/sector hardcoding, and it must balance theme evidence/purity, source role support, investability/data quality, liquidity/scale, preliminary financial sanity, and representative coverage. Representative coverage can diversify only among qualified theme-fit candidates; weak-adjacent, unrelated, or unsupported candidates must not be forced into the universe just to fill configured slots. Update/improve diagnostics on a locked research report must reuse the persisted role evidence, keep the full locked universe as the report/table/graph universe, and expose a separate qualified subset for allocation and conclusion. Research reports must always include a dependency/role map; when the LLM enrichment pass is skipped, render the deterministic provider-profile role map instead of omitting the section.
...
- `RESEARCH_UNIVERSE_MIN_THEME_SCORE` — core theme-evidence/fit gate for research universe selection (0–100, default 70)
- `RESEARCH_UNIVERSE_STRONG_ADJACENT_THEME_SCORE` — strong-adjacent theme-evidence/fit gate (0–core gate, default 55)
