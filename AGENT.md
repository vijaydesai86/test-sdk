...
Deep-sector research resolves a broad live provider-confirmed candidate pool through generic theme role buckets. The LLM may propose required theme dimensions, role buckets, company names, and likely tickers with role evidence, but every symbol must still be provider-verified or searched by company name before data fetching. Broad resolver output is raw discovery only: it must not appear as a final role, qualify allocation, or lock a universe until provider/LLM classification ties the company to a concrete `direct` or `enabler` role. The selector persists role evidence, required dimensions, readiness status (`discovering`, `refining`, `locked`, `failed`), theme qualification metadata (`direct`, `enabler`, `beneficiary`, `unrelated`), and a structured research pipeline checkpoint that records stage, pending tasks, completed tasks, missing dimensions, target final count, current shortlist, and next objective. A `locked` universe is stable and preserved by later update/improve passes. A `refining` universe may still publish a full market-backed provisional research report when it has enough provider-confirmed direct/enabler candidates and concrete role coverage to be useful; the report must clearly show provisional readiness diagnostics, missing dimensions, and that later improve passes may keep refining the universe. Strong but role-incomplete refining universes should still fetch core market data and render a provisional report rather than falling back to an empty discovery-only artifact. Only `discovering`/`failed` or too-thin refining states should fall back to a discovery-only checkpoint. Critical market data (price, overview, basic financials, price history) is fetched before optional ecosystem/dependency LLM enrichment. Universe scoring must remain generic: no production ticker/sector hardcoding, and it must balance theme evidence/purity, source role support, investability/data quality, liquidity/scale, preliminary financial sanity, and representative coverage. Representative coverage prefers qualified theme-fit candidates, but best-effort provider-validated candidates may fill provisional report slots to avoid empty reports; weak-adjacent, beneficiary-only, broad-only, unrelated, or unsupported candidates must remain unqualified and must not drive allocation, conclusion, or lock readiness. Update/improve passes on an unlocked/provisional research report must resume discovery/refinement from persisted metadata and keep the configured target universe size rather than shrinking to the partial candidate count; update/improve diagnostics on a locked research report must reuse the persisted role evidence, keep the full locked universe as the report/table/graph universe, and expose a separate qualified subset for allocation and conclusion. An improve pass must not be accepted as progress unless it improves readiness, locks/provisions the universe, changes the universe meaningfully, or fills real provider-backed data; repeated no-op `research_universe_still_unready` passes must stop instead of burning the configured pass budget. Research reports must always include a dependency/role map; when the LLM enrichment pass is skipped, render a deterministic role map from concrete query-derived/provider-grounded theme roles instead of omitting the section. Provider sector/industry groups are diagnostic metadata only: they must not create role-coverage bonuses, qualify lock/allocation, or appear as real dependency roles. If concrete theme roles are unavailable, keep the configured provider-validated company universe non-empty and map those companies under a provisional/unclassified bucket until later improve passes repair role evidence. Unlocked research improve passes must use the persisted progress checkpoint: retry temporary failures, continue from the cursor, basic-score the next candidate batch, re-rank all scored candidates, and accept replacements only when the updated report does not lose company count or verified data.
...
- Internal fallback chain: gemini-2.5-flash → gemini-2.5-flash-lite; `gemini-2.5-pro` joins only when explicitly requested or `GEMINI_ENABLE_PRO_FALLBACK=true`
- Gemini OpenAI-compatible calls may set `reasoning_effort` from `GEMINI_CHAT_REASONING_EFFORT` / `GEMINI_FILL_REASONING_EFFORT` (`low`, `medium`, `high`; default `low`)
...
- `FILL_MODEL`, `GEMINI_MODEL`, `GEMINI_FALLBACK_MODELS`, `GEMINI_ENABLE_PRO_FALLBACK`
- `GEMINI_CHAT_REASONING_EFFORT`, `GEMINI_FILL_REASONING_EFFORT`
- `AUTO_DOWNGRADE_GPT5` (default `true`)

**Scaling:**
- `NUM_COMPANIES` — companies per sector/comparison report (2–15, default 10)
- `RESEARCH_CANDIDATE_POOL_MULTIPLIER` — fresh research candidate pool multiplier before scoring (1–6, default 5)
- `RESEARCH_THEME_FACET_COUNT` — maximum generic concrete theme role buckets generated for candidate discovery (1–10, default 6)
- `RESEARCH_FACET_CANDIDATES` — candidate company/ticker ideas requested per role bucket (2–15, default 8)
- `RESEARCH_UNIVERSE_MIN_THEME_SCORE` — core theme-evidence/fit gate for research universe selection; full reports also require concrete role readiness (0–100, default 70)
- `RESEARCH_UNIVERSE_STRONG_ADJACENT_THEME_SCORE` — strong-adjacent theme-evidence/fit gate; beneficiary-only and broad-only evidence cannot qualify (0–core gate, default 55)
